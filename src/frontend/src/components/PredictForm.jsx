...
export default function PredictForm() {
  ...
  const handleChange = (event) => { const { name, value, type, checked } = event.target; setForm((current) => ({ ...current, [name]: type === "checkbox" ? (checked ? 1 : 0) : value })); };
  ...
  return <main className="mx-auto w-full max-w-[1380px] px-4 py-6">
    ...
    {step === 2 && <form onSubmit={submit} className="mx-auto max-w-4xl space-y-4">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-start justify-between gap-4"><div><span className="text-xs font-semibold text-blue-700">STEP 2</span><h2 className="mt-2 text-xl font-semibold text-slate-900">Property information</h2><p className="mt-1 text-sm text-slate-500">Details for {selectedLocation?.label}.</p></div><button type="button" onClick={() => setStep(1)} className="text-xs font-semibold text-slate-500">Change location</button></div>
        <div className="mt-6 grid gap-5 md:grid-cols-2">
          <div className="rounded-xl bg-slate-50 p-4"><h3 className="text-sm font-semibold text-slate-800">Property size</h3><label className="mt-3 block text-xs font-medium text-slate-600">Land area in square feet<input aria-label="Land area" type="number" min="1" name="land_area" value={form.land_area} onChange={handleChange} required placeholder="e.g. 1369" className={inputClass} /></label><p className="mt-2 text-xs text-slate-400">1 Aana ≈ 342.25 sq ft{form.land_area && Number(form.land_area) > 0 ? ` · approximately ${(Number(form.land_area) / 342.25).toFixed(2)} Aana` : ""}</p></div>
          <div className="rounded-xl bg-slate-50 p-4"><h3 className="text-sm font-semibold text-slate-800">Building</h3><div className="mt-3 grid grid-cols-3 gap-2">{[["bedroom", "Bedrooms"], ["bathroom", "Bathrooms"], ["floor", "Floors"]].map(([name, label]) => <label key={name} className="text-xs font-medium text-slate-600">{label}<input aria-label={label} type="number" min="0" step={name === "floor" ? "0.5" : "1"} name={name} value={form[name]} onChange={handleChange} required className={inputClass} /></label>)}</div></div>
          <div className="rounded-xl bg-slate-50 p-4"><h3 className="text-sm font-semibold text-slate-800">Accessibility and age</h3><div className="mt-3 grid grid-cols-2 gap-3"><label className="text-xs font-medium text-slate-600">Road access<select aria-label="Road access" name="road_access" value={form.road_access} onChange={handleChange} className={inputClass}>{ROAD_ACCESS_OPTIONS.map((value) => <option key={value} value={value}>{value} ft</option>)}</select></label><label className="text-xs font-medium text-slate-600">Property age<input aria-label="Property age" type="number" min="0" name="property_age" value={form.property_age} onChange={handleChange} required placeholder="Years" className={inputClass} /></label></div><label className="mt-3 block text-xs font-medium text-slate-600">Facing direction<select aria-label="Facing direction" name="facing_encoded" value={form.facing_encoded} onChange={handleChange} className={inputClass}>{FACING_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div>
        ...
        {error && <p role="alert" className="mt-4 text-sm text-red-600">{error}</p>}<button type="submit" disabled={loading} className="mt-6 w-full rounded-lg bg-slate-900 py-3 text-sm font-semibold text-white disabled:opacity-50">{loading ? "Analysing property..." : "Analyse Property"}</button>
      </section>
    </form>}
