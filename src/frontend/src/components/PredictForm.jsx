...
export default function PredictForm({ step = 1, onNavigate = () => {} }) {
  ...
  return <main className="mx-auto w-full max-w-7xl px-6 py-8 lg:py-10">
    <div className="mb-8"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-700">Property analysis</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950 lg:text-4xl">Understand a property and its location</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">Estimate its present value and review nearby infrastructure in three simple steps.</p></div>
    <StepProgress step={step} />

    {step === 1 && <section className="grid items-stretch gap-5 lg:grid-cols-[340px_minmax(0,1fr)]">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><span className="inline-flex rounded-full bg-blue-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-blue-700">Location details</span><h2 className="mt-4 text-xl font-semibold text-slate-900">Where is the property?</h2><p className="mt-2 text-sm leading-6 text-slate-500">Choose a recognised area, then fine-tune the exact point on the map.</p>
        ...
        <div className="mt-5 rounded-xl border border-slate-800 bg-slate-50 p-4"><p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Selected area</p><p className="mt-1.5 text-sm font-semibold text-slate-700">{selectedLocation?.label || "No area selected yet"}</p>{propertyPoint && <p className="mt-2 font-mono text-[11px] text-slate-500">{propertyPoint.lat.toFixed(6)}, {propertyPoint.lon.toFixed(6)}</p>}</div>
        <p className="mt-4 text-xs leading-5 text-slate-500">The precise point is used only for nearby infrastructure analysis.</p>{error && <p role="alert" className="mt-3 text-sm text-red-600">{error}</p>}
      </div>
